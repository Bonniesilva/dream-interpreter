...
description: 解梦技能（周公解梦+心理学双轨）。触发词：解梦、周公解梦、帮我解梦、我做了个梦、我昨晚梦到、我梦见、梦到、做梦、梦境、今日运势、抽签、梦境签、梦境图、生成梦境图、出图、播报。
allowed-tools: message, bash, tts
---
...
**第一步：立即用 `message` 工具发送确认消息：**
```json
{"action": "send", "channel": "telegram", "message": "🌙 解梦技能已启动，正在为你解读……"}
```

**第二步：完成解读后，用 `message` 工具发送完整报告正文（纯文字，无按钮）。**

**第三步：紧接着再调用一次 `message` 工具，发送按钮（无文字）：**
```json
{
  "action": "send",
  "channel": "telegram",
...
**�🚨 第三步是强制的，不可省略。报告发完后必须立即发出这条按钮消息，不得用任何普通文字代替。**
...
**干支年柱计算规则（必须严格执行）：**
- 年柱：根据当前公历年份换算天干地支（2025=乙巳，2026=丙午，2027=丁未）
- 日柱：根据当日公历日期推算当日天干地支
- 五行：天干地支对应五行（甲乙=木，丙丁=火，戊己=土，庚辛=金，壬癸=水；子亥=水，寅卯=木，巳午=火，申酉=金，辰戌丑未=土）
- 农历日期：根据公历换算（必须准确，不能瞎填）
...
昨夜梦境五行：水　与今日土克水，需注意情绪消耗
...
```

**🚨 运势输出铁律：运势报告必须通过 `message` 工具发送，且必须在同一条消息中携带 buttons 参数：**
...
> ❌ **绝对禁止**收到出图请求后直接生成图片
> ❌ **绝对禁止**跳过风格选择步骤
> ✅ 收到任何出图请求（按钮点击或文字触发）**必须先发送6个风格选择按钮**
> ✅ 等用户点击风格按钮后，才能开始生成图片
> ✅ 整个流程：收到出图请求 → 发风格选择按钮 → 用户选择风格 → 生成图片

**收到 `dream_image` callback 或出图触发词时，第一步永远是发送风格选择，没有例外。**

> ⚡ **即时响应铁律**：收到出图请求后，**不做任何思考、不调用任何工具、不生成任何内容**，直接立刻调用 `message` 工具发出风格选择按钮。风格选择必须是响应出图请求的**第一个也是唯一一个**动作。

触发时机：
- 用户点击「🎨 生成梦境图」按钮（callback_data: dream_image）
- 用户说「梦境图」「生成图片」「画出我的梦」「看看我的梦长什么样」

**第一步：立即发送风格选择（不得有任何前置动作）**
...
**🚨 抽签输出铁律：签文必须通过 `message` 工具发送，且必须在同一条消息中携带 buttons 参数：**
